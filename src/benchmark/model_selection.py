...
def run_selection(
    ...
    *dataset_filter* restricts candidates to a single dataset
    ("80" for COCO-80, "person" for COCO-Person, "merged" or "fyp_merged" for FYP-Merged).
    When set, the
...
def _cli_entry(
    ...
    dataset: str | None = typer.Option(
        None,
        "--dataset",
        help="Filter dataset: '80', 'person', 'merged', or 'fyp_merged' (FYP-Merged).",
    ),
    ...
    if dataset is not None and dataset not in _DATASET_LABELS:
        _err.print(
            f"[red]Error: --dataset must be '80', 'person', 'merged', or 'fyp_merged', got '{dataset}'[/red]"
        )
        raise typer.Exit(2)
